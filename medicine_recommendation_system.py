...
# Load Model 
model = joblib.load(r"E:\Desktop\Project_Folder\models\SVC_Model.joblib")
...
# Detect if the program is running as a frozen executable
if getattr(sys, 'frozen', False):
    # If frozen, the path to the file is located in the temporary directory (PyInstaller's _MEIPASS)
    base_path = sys._MEIPASS
else:
    # If running as a script, use the current directory
    base_path = os.path.dirname(__file__)

# Construct the correct path for your dataset files
description_file = os.path.join(base_path, 'Dataset', 'description.csv')
diets_file = os.path.join(base_path, 'Dataset', 'diets.csv')
medications_file = os.path.join(base_path, 'Dataset', 'medications.csv')
precautions_file = os.path.join(base_path, 'Dataset', 'precautions_df.csv')
symptoms_file = os.path.join(base_path, 'Dataset', 'symtoms_df.csv')
training_file = os.path.join(base_path, 'Dataset', 'Training.csv')
workout_file = os.path.join(base_path, 'Dataset', 'workout_df.csv')

# Load the CSV files using the corrected path
description = pd.read_csv(description_file)
dieat = pd.read_csv(diets_file)
medications = pd.read_csv(medications_file)
precautaion = pd.read_csv(precautions_file)
...
def medicine_recommendation():
    ...
    heading = Label(app, text = "Medicine AI Base Recommendation System",font = ("Robot",30, "bold"), fg="white", bg="#055B7F", justify = "left")
